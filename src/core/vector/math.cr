...
module Bottle::Core::VectorMath
  ...
  # v1 + v2 # => [3.0, 6.0, 9.0]
  # ```
  def add(a : Vector(GslVector, Float64), b : Vector(GslVector, Float64))
  ...
  # v1 + v2 # => [3.0, 4.0, 5.0]
  # ```
  def add_constant(a : Vector(GslVector, Float64), x : Number)
  ...
  # v1 - v2 # => [-1.0, -2.0, -3.0]
  # ```
  def sub(a : Vector(GslVector, Float64), b : Vector(GslVector, Float64))
  ...
  # v1 - v2 # => [-1.0, 0.0, 1.0]
  # ```
  def sub_constant(a : Vector(GslVector, Float64), x : Number)
    LibGsl.gsl_vector_add_constant(a.ptr, -x)
    return a
  end
  ...
  # v1 * v2 # => [3.0, 8.0, 18.0]
  # ```
  def mul(a : Vector(GslVector, Float64), b : Vector(GslVector, Float64))
  ...
  # v1 + v2 # => [2.0, 4.0, 6.0]
  # ```
  def mul_constant(a : Vector(GslVector, Float64), x : Number)
  ...
  # v1 / v2 # => [0.5, 0.5, 0.5]
  # ```
  def div(a : Vector(GslVector, Float64), b : Vector(GslVector, Float64))
  ...
  # v1 / v2 # => [0.5, 1, 1.5]
  # ```
  def div_constant(a : Vector(GslVector, Float64), x : Number)
