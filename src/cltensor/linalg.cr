...
class ClTensor(T)
  macro blast(name, *args, prefix = "")
    ...
    queue = NumInternal::ClContext.instance.queue
  ...
  def scal!(a : T)
    blast(scal, size, a, buffer, 0, 1)
  end

  def axpy!(a : T, x : ClTensor(T))
    blast(axpy, size, a, x.buffer, 0, 1, buffer, 0, 1)
  end
  ...
  def dot(other : ClTensor(T), ret : ClTensor(T)? = nil)
    ...
    blast(dot, size, back.buffer, 0, buffer, 0, 1, other.buffer, 0, 1)
    back
  end
  ...
  def nrm2(ret : ClTensor(T)? = nil)
    ...
    blast(nrm2, size, back.buffer, 0, buffer, 0, 1)
    back
  end
  ...
  def asum(ret : ClTensor(T)? = nil)
    ...
    blast(asum, size, back.buffer, 0, buffer, 0, 1)
    back
  end
  ...
  def sum(ret : ClTensor(T)? = nil)
    ...
    blast(sum, size, back.buffer, 0, buffer, 0, 1)
    back
  end
  ...
  def amax(ret : ClTensor(Int32)? = nil)
    ...
    blast(amax, size, back.buffer, 0, buffer, 0, 1, prefix: i)
    back
  end
  ...
  def amin(ret : ClTensor(Int32)? = nil)
    ...
    blast(amin, size, back.buffer, 0, buffer, 0, 1, prefix: i)
    back
  end
  ...
  def argmax(ret : ClTensor(Int32)? = nil)
    ...
    blast(max, size, back.buffer, 0, buffer, 0, 1, prefix: i)
    back
  end
  ...
  def argmin(ret : ClTensor(Int32)? = nil)
    ...
    blast(min, size, back.buffer, 0, buffer, 0, 1, prefix: i)
    back
  end
  ...
  def matmul(other : ClTensor(T), ret : ClTensor(T)? = nil)
    ...
    blast(gemm, LibBlast::CLBlastLayout::CLBlastLayoutRowMajor,
      ...
      LibBlast::CLBlastTranspose::CLBlastTransposeNo,
      shape[0], other.shape[1], shape[1], 1.0, buffer,
      0, shape[1], other.buffer, 0, other.shape[1], 1.0,
      back.buffer, 0, back.shape[1])
